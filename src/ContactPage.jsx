...
const ContactPage = ({ initialSection = 'enquiry' }) => {
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white pt-40 pb-24 px-6 md:px-24 relative overflow-hidden">
            {/* Background Ambience */}
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-12">
                    ...
                    <div className="space-y-4">
                        {CONTACT_SECTIONS.map((section) => (
                            <motion.div
                                ...
                                className={`p-8 cursor-pointer transition-all duration-500 relative group overflow-hidden ${activeSection === section.id ? 'bg-white/10' : 'bg-white/5 hover:bg-white/8'
                                    }`}
                                whileHover={{ x: 10 }}
                            >
                                <div className="relative z-10 flex items-center justify-between">
                                    <h3 className={`text-lg font-logo tracking-[0.2em] uppercase ${activeSection === section.id ? 'text-[#C5A059]' : 'text-white/40 group-hover:text-white/70'
                                        }`}>
                                        {section.label}
                                    </h3>
                                    <section.icon size={20} className={activeSection === section.id ? 'text-[#C5A059]' : 'text-white/20'} />
                                </div>
                    ...
                    <motion.div
                        ...
                        className="bg-white/5 backdrop-blur-xl border border-white/5 p-12 md:p-16 relative"
                    >
                        <AnimatePresence mode="wait">
                            {activeSection === 'enquiry' && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                    <h2 className="text-3xl font-logo uppercase tracking-widest mb-4">Quick Enquiry</h2>
                                    <p className="text-white/40 text-sm mb-12 font-sans tracking-wide">
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-8">
                                        ...
                                        <button type="submit" disabled={isSubmitting} className="group relative px-10 py-5 bg-[#C5A059] text-black text-[10px] font-bold tracking-[0.3em] uppercase transition-all duration-300 hover:bg-white">
                            ...
                            {activeSection === 'location' && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col justify-center">
                                    <h2 className="text-3xl font-logo uppercase tracking-widest mb-8">Contact Us</h2>
                                    <div className="grid md:grid-cols-2 gap-12">
                                        <div className="space-y-6">
                                            ...
                                            <div className="space-y-3 pt-6 border-t border-white/10">
                                                <p className="flex items-center gap-4 text-white/50 hover:text-[#C5A059] transition-colors text-sm tracking-wide">
                                        ...
                                        <div className="h-[500px] bg-white/5 border border-white/10 overflow-hidden relative group">
                                            <iframe
                                                title="location-map"
                            ...
                            {activeSection === 'vendor' && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                    <h2 className="text-3xl font-logo uppercase tracking-widest mb-4">Vendor Registration</h2>
                                    <p className="text-white/40 text-sm mb-12 font-sans tracking-wide">Join our network of premium suppliers and contractors.</p>
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-8">
                                        ...
                                        <button type="submit" disabled={isSubmitting} className="group relative px-10 py-5 bg-white text-black text-[10px] font-bold tracking-[0.3em] uppercase transition-all duration-300 hover:bg-[#C5A059] hover:text-white">
                                            <span className="flex items-center gap-3">
