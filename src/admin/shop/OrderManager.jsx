...
const OrderDetailModal = ({ order: initialOrder, pickupLocations, onClose, onRefresh }) => {
    ...
    const handleSendWhatsApp = () => {
        const phone = order.customer_phone.replace(/[^0-9]/g, '');
        const message = `Hi ${order.customer_name},\n\nYour order *${order.order_number}* from Adroit Design Studio has an update.\n\nStatus: *${order.order_status.replace(/_/g, ' ').toUpperCase()}*` + 
            (order.tracking_url ? `\n\nTrack your shipment here: ${order.tracking_url}` : '') + 
            `\n\nThank you for shopping with us!`;
        window.open(`https://wa.me/91${phone.slice(-10)}?text=${encodeURIComponent(message)}`, '_blank');
    };

    const isShipped = ['shipped', 'out_for_delivery', 'delivered', 'returned'].includes(order.order_status);
                ...
                <div className="p-6 space-y-6">
                                    ...
                                    >
                                        {pickupLocations && pickupLocations.length > 0 ? (
                                            pickupLocations.map(loc => (
                                                <option key={loc.id} value={loc.pickup_location} className="bg-[#111]">
                                                    {loc.pickup_location} ({loc.address}, {loc.city})
                                                </option>
                                            ))
                                        ) : (
                                            <option value="Franklin" className="bg-[#111]">Franklin (Default)</option>
                                        )}
                                    </select>
                                    {pickupLoc && (
                    ...
                    <div className="border-t border-white/10 pt-4 flex gap-3">
                        <button
                            ...
                            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] font-bold text-xs uppercase tracking-wider rounded-xl transition-all border border-[#25D366]/20 flex-1"
                        >
...
const OrderManager = () => {
    ...
    const fetchPickupLocations = async () => {
        try {
            const res = await pickupLocationsAPI.getShiprocketLocations();
